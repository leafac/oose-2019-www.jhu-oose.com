...
We’ll add a checklist of the specific things you need to do after the lecture, as we’ll be making some decisions live. Check back here on Thursday (2019-10-03). Here’s a partial list:

- Start the work using the code base as you left it in [Assignment 4](/assignments/4), on the `show-completed` branch.
- Fetch `origin` and merge the changes from [Lecture 5](/lectures/5), which are on the `origin/show-completed` branch. If you run into merge conflicts, particularly on the Postman JSON file, accept the version from `origin` and let it overwrite yours.
- Work on all the `TODO`s we left from lecture. **(This is the part of the checklist that we’ll flesh out after the lecture.)**
- Observe that the tests (including the new tests you added) are passing on Travis CI.
